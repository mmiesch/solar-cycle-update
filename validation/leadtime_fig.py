"""
This is similar to leadtime_fromjson.py but it is was used for making a figure for the paper so both plots are included in the same figure.
"""
import datetime
import json
import numpy as np
...
# Figure product
outfig = outdir + '/leadtime_fig.png'
...
ax[0].set_ylabel('SSN', fontweight='bold', fontsize=12)
ax[0].set_xlabel('Date', fontweight='bold', fontsize=12)

ax[0].set_xlim([datetime.date(2022,12,1),datetime.date(obstime[-6].year,obstime[-6].month,1)])
...
ax[1].set_ylabel('F10.7 (sfu)', fontweight='bold', fontsize=12)
